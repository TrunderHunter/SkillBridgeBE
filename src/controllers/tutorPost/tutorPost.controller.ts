...
export class TutorPostController {
  ...
  async searchTutorPosts(req: Request, res: Response) {
    try {
      ...
      logger.info(
        `Search completed: ${result.posts.length} posts found`,
        {
          query,
  ...
  async getDistrictsByProvince(req: Request, res: Response) {
    try {
      ...
      const districts = await tutorPostService.getDistrictsByProvince(provinceCode);
  ...
  async getTutorPostById(req: Request, res: Response) {
    try {
      ...
      return sendSuccess(
        res,
        'Tutor post retrieved successfully',
        { tutorPost }
      );
    } catch (error) {
